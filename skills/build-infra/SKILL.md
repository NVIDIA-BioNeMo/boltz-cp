...
name: build-infra
description: >
  Probe hardware, software, and multi-rank collectives for context-parallel development. Use to establish a local or SLURM launch recipe before implementing or testing CP code.
...
# build-infra — stand up and verify the CP test environment
...
- **`PYTHONPATH=<repo root>`** when the model is not pip-installed — `mp.spawn` (spawn
  ...
  without it.
- **Sandbox / seccomp can block NCCL.** The AF_UNIX sockets used by process-group
  bootstrap may be unavailable in the current runtime. Diagnose with the bounded smoke
  tests, then use an already authorized GPU runner or SLURM allocation that supports NCCL.
  If none is available, report the missing runtime capability and wait for access.
  Preserve the host's security settings and record the supported launch recipe.
...
- **Per-step hardware (R10) is runtime-derived, not a fixed table.** GPU **count → testable
  ...
  budgets** come from `dtensor-modules` (the O(N/cp0) / O(N²/(cp0·cp1)) backward budget)
  and are measured by `mem-profile`; **training vs inference** differ only by fwd vs
  fwd+bwd+step, captured by `benchmark` on the *same* mesh. Record the SKU you actually
