---
name: cpize-model-workflow
description: >
  Coordinate end-to-end context-parallel integration into a biomolecular model, from model mapping through correctness gates and profiling. Use for a whole-model port; use dtensor-modules for one module.
...
# cpize-model-workflow — conduct the whole CP integration, prioritized and gated
...
|---|---|---|---|
| 0 | Map the model | `learn-context` | `docs/current_code_structure.md` filled (module map + feature inventory) |
| 1 | Verify infra | `build-infra` | `docs/cp_infra.md`; resource + hardware/runtime **self-consistency** recorded; collective + **DTensor/`device_mesh`** smoke tests pass |
| 2 | Shard data features | `shard-data-feats` | data-feature parity test green |
| 3 | Port modules | `dtensor-modules` (+ `dispatch-work` to fan out) | each module's parity test green |
| 4 | Wire trainer/predictor | `dist-lifecycle` | stop-and-go resume test green |
| 5 | Prove correctness | `test` | end-to-end CP==serial parity + property tests green, non-vacuous |
| 6 | Profile | `mem-profile`, `nsys-profile` | peak ≤ budget (Rule 11); compute/comm attributed |
| 7 | Benchmark | `benchmark` | max-token / walltime recorded |
...
| bare `2d` | **topology** hint only (sizes deferred to `build-infra`) | from the `cp:` shape |
| `model:<path>` | the **user's model code** | resolve by tracing |
| `ref:<path>` | the **reference repo** (`$BOLTZ_CP_REPO`) | `$BOLTZ_CP_REPO` env / search |
| `--local` / `--slurm` | **launch environment** | `build-infra` decides |
...
- **Reference repo (`$BOLTZ_CP_REPO`)** — the Boltz-CP implementation every mapping cites. Resolve
  it first (env var → `ref:<path>` → search); phase-0 `learn-context` prompts if unset. Record it
  in the ledger — *the whole workflow is undefined without it.* The **model path** (`model:<path>`)
  and **scope** feed phase-0 `learn-context`; the **launch** flag feeds phase-1 `build-infra`.
- **Mesh config — the concrete 2D device mesh used to TEST, BENCHMARK, and PROFILE.**
  ...
  sets the Step-4 slot bound (`slots = floor(free_GPUs / world_size)`). Absent ⇒ `build-infra`
...
- If `docs/current_code_structure.md` is missing/stale, run `learn-context` first
  (phase 0) — never plan module work against an un-mapped model (Rule 1). If `docs/cp_infra.md` is
  missing, run `build-infra` (phase 1).
- Read the **§4 module map**, the **§3 data-feature inventory**, and the **§4.5
  ...
  §4.5 is missing/stale, re-run `learn-context`. **Thread each module task's input-feature
  placement contract from §4.5** so a `dtensor-modules` port wires against a *known* contract, and
  make the **synthetic test data** (`test`) emit at those same placements by reusing the
...
**first**, and the subsystem is the natural scope to hand to `dispatch-work`. This does
...
hand the wave to `dispatch-work` (coder+reviewer pairs, GPU-slot-bounded). 1–2 items →
`dtensor-modules` + `test` directly. (`dispatch-work` sorts *within* the
...
1. Pick the next **ready** wave — dependencies met, fits GPU slots
   ...
   parallel tests otherwise overload the GPUs and stall (see `dispatch-work`); under SLURM
   the scheduler sets per-job device visibility.
2. Delegate to the phase's skill (or `dispatch-work` for a fan-out wave).
...
writes → **1×1-mesh fp64 pre-check** (`test` Step 3) → multi-rank parity → iterates to
...
Route to `test` for the **end-to-end workflow parity** (CP fwd+bwd == serial) on top of
...
Run `mem-profile`, then `nsys-profile` (they may overlap if GPUs allow). Read
...
`benchmark` for max-token-at-CP-size and end-to-end walltime → `docs/cp_benchmark.md`.
...
- A final summary: what was reused vs newly built, the correctness evidence, the memory/compute
  headline numbers, and any remaining `TODO(learn-context)` / follow-ups.
