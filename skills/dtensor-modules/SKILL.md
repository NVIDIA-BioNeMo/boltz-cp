---
name: dtensor-modules
description: >
  Port a serial biomolecular model module to DTensor context parallelism with explicit collectives and bounded backward memory. Use for layer, attention, diffusion, confidence, or loss implementations.
...
# dtensor-modules — port a serial module to context parallelism
...
- The features the module consumes are (or will be) produced by
  `shard-data-feats` at known placements.
...
against YOUR data placements (from `shard-data-feats` / `current_code_structure.md` §3)
...
- the **complete input contract** — including feature-dict fields the signature
  ...
  `shard-data-feats`. An undocumented feature input at the wrong placement is the
...
- Decorate `@torch.amp.custom_fwd(device_type="cuda")` / `custom_bwd`, then **audit the
  ...
  up/down-cast (correctness-silent but a transient peak-memory hazard — see `mem-profile`).
...
job `dispatch-work` enforces between paired agents.

## Step 6 — Test against the serial reference

Hand to `test` at the right level:
...
- **module integration** — the module with its real feature inputs drawn from the **shared
  ...
  also exercises the featurizer→module seam (Rule 4) — see `test` Step 2.2;
