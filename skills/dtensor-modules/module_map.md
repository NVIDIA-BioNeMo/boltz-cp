# dtensor-modules — serial → CP dictionary

Look up the target here before implementing. All paths are relative to
`src/boltz` in the repository selected by `BOLTZ_CP_REPO`. Read the cited tech guide section for the algorithm
...
precondition the data pipeline must satisfy (`shard-data-feats`), and it is
...
**required placement** that must match what `shard-data-feats` produced. Before
...
- **Pin the communicated-buffer dtype across ranks** (Rule 19): if a buffer's dtype
  ...
  *unintended* up/down-cast is correctness-silent (parity passes after the round-trip) but
  ...
  dtype and verify peak with `mem-profile`.
