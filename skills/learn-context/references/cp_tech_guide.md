...
Source links point into this repository. When using an installed copy of the skill,
resolve `src/` and `tests/` paths against the `BOLTZ_CP_REPO` recorded by
`learn-context`. Interactive HTML demonstrations from the original guide are not
bundled; the algorithms and Mermaid diagrams are included below.
