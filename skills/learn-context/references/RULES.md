...
1. **Explore before integrating.** Every fold-cp task begins from
   `docs/current_code_structure.md` (the `learn-context` output) and the
   located Boltz-CP reference repo plus the bundled [CP technical guide](cp_tech_guide.md).
   If `current_code_structure.md` is
   missing or stale, run `learn-context` first. Never write CP code against an
   un-mapped model.
...
> distribution** so the sharded axis splits evenly (`shard-data-feats`); and a
...
10. **Reduce gradients in ≥fp32; use `torch.promote_types(dtype, torch.float32)`,
    ...
    intermediates in the intended dtype and verify peak with `mem-profile`.
11. **Backward memory must match the forward per-rank budget.** Single/token
    ...
    *recompute* (`mem-profile`), and apply any sequence-length cap **before** the
...
18. **Distributed lifecycle is part of correctness.** Move modules to device **before**
    ...
    (in-place `.data` math bypasses DTensor dispatch). See `dist-lifecycle`.
19. **Audit the dtype path of every `autograd.Function`.** Decorate it
    ...
    to parity tests (catch it with `mem-profile`). Treat casts as
...
20. **Every rank must reach every collective — guard rank-asymmetric work.** A collective
    ...
    *and* DP, not just CP** — pad missing keys with sentinels so all ranks issue an
    ...
    distributed-liveness concern (`shard-data-feats`). Use **`try/finally` for cleanup**
    ...
    finished job/agent does not leak GPU memory (the `dispatch-work` shutdown protocol
...
> guard when the optional plugin is installed — write `.fold-cp/config.json`
> (see `learn-context`) to enable it.
