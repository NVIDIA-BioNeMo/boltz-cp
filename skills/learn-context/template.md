...
> Generated by `learn-context` on <date>. This is the lookup reference
> for all downstream fold-cp skills. Keep it current: re-run `learn-context` after
...
| Tech guide | `<absolute path to learn-context/references/cp_tech_guide.md>` |
| Distributed code to mirror | `<BOLTZ_CP_REPO>/src/boltz/distributed/` |
| Governing rules | `<absolute path to learn-context/references/RULES.md>` |
...
- `TODO(learn-context): <unresolved question>`

## 7. Recommended next steps

- `<build-infra | shard-data-feats | dtensor-modules>` because `<reason>`.
