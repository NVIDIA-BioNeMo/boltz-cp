...
name: learn-context
description: >
  Map a biomolecular model’s entry points, data features, and serial modules onto the Boltz-CP reference. Use before CP integration or when the model inventory is stale; produces docs/current_code_structure.md.
...
# learn-context — map the user's model and bind it to the Boltz-CP reference

Read the shared [CP integration rules](references/RULES.md) before starting.
...
The primary CP reference is bundled with this skill:

- **[references/cp_tech_guide.md](references/cp_tech_guide.md)** — per-module CP algorithms, I/O
...
(`$BOLTZ_CP_REPO/src/boltz/distributed/`). The shared CP rules are
[references/RULES.md](references/RULES.md); read them when using standalone skills.
The optional plugin also loads this file at SessionStart. Record the absolute
paths to both references in `docs/current_code_structure.md` for downstream skills.
...
as new work for `dtensor-modules`.
...
1. Write `docs/current_code_structure.md` from
   ...
   `TODO(learn-context): <question>` markers for unresolved gaps rather than
   guessing.
...
- Report a one-paragraph summary: what reuses existing CP components vs what is new
  work, and the recommended next skill (`build-infra` if infra is unverified, else
  `shard-data-feats`).
