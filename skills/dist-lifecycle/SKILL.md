...
name: dist-lifecycle
description: >
  Integrate CP modules with a trainer or predictor: device placement, optimizer state, checkpoint save/load, resume, and EMA. Use after module parity passes to verify lifecycle behavior.
...
# dist-lifecycle — make a CP model train, checkpoint, and resume correctly

Read the shared [CP integration rules](../learn-context/references/RULES.md) before starting.

Porting modules (`dtensor-modules`) proves the math; this skill makes the *trainer*
...
- Modules ported + parity-tested (`dtensor-modules` + `test`).
...
Hand to `test`: train N steps → checkpoint → resume → train M more, and assert
