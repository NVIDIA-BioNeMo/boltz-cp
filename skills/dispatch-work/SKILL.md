...
name: dispatch-work
description: >
  Schedule coder and reviewer agents across independent CP module tasks and reconcile coupled placement contracts. Use for a multi-module implementation wave after its dependencies are mapped.
...
# dispatch-work — parallelize CP integration with coder+reviewer pairs
...
module, call `dtensor-modules` + `test` directly. Roles, the
...
1. Collect every module/test marked **adapt** or **new** in §4 (plus any data-feature
   work still open from `shard-data-feats`).
...
- **coder** — **owns the CP module *and* its test**; implements per `dtensor-modules` /
  `test`, loads the module's row from `module_map.md`, the serial twin, and the relevant
  tech-guide §. **Verifies the fwd+bwd decomposition on a 1×1 mesh (fp64) before the multi-rank
  parity run** (`test` Step 3) and **runs the test green before handoff** — this makes the
