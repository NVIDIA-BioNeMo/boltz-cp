# dispatch-work — coordination reference

## Roles per work item

| Role | Does | Loads |
|---|---|---|
| **coder** | **owns** the CP module **and** its test; runs the parity test **green** before handing off | `dtensor-modules` or `test`; `module_map.md` row; serial twin; tech-guide § |
...
LOAD SKILL: <dtensor-modules | test> (+ team_swarm:intercomm if present)
