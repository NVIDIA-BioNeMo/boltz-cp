...
In the reference repository selected by the `BOLTZ_CP_REPO` environment variable,
reuse `src/boltz/testing/utils.py` and `src/boltz/distributed/testing/utils.py`.
...
Use the [comparison rules](SKILL.md#step-4--compare-against-serial-the-right-reassembly)
and [anti-vacuous assertions](SKILL.md#step-5--anti-vacuous-assertions-all-required)
when adapting the skeleton. The [run protocol](SKILL.md#step-7--run-with-timeout-and-logs)
defines the evidence the test must print. Memory correctness also needs the
dtype-path checks below; numerical parity alone cannot detect transient upcasts.
...
- **Negative control (mandatory):** re-run against the known-bad code in an isolated
  temporary checkout and confirm the assertion FAILS; keep the user's work intact.
...
For the property-test acceptance criteria, use the
[mandatory checks in SKILL.md](SKILL.md#a-second-test-class-property--invariant-tests-not-serial-parity).
