...
name: test
description: >
  Verify CP correctness with serial-reference forward/backward parity and distributed invariant tests. Use after feature sharding or module changes; covers meaningful gradients, active sharding, and RNG entropy.
...
- `docs/cp_infra.md` gives the launch recipe + `world_size` (run `build-infra` if
  not).
...
- **Negative control is mandatory.** Prove the test catches the bug: run it once against the
  known-bad implementation in an isolated temporary checkout and confirm it **FAILS**.
  Keep the user's working tree and uncommitted changes intact.
...
- **Checkpoint / serialization changes:** cover **both save and load**, including
  **backward-compatibility with the prior format** (load an old-format checkpoint), not just
  a round-trip of the new format. (See `dist-lifecycle` for the resume test.)
