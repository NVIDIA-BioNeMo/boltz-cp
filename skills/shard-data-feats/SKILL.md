---
name: shard-data-feats
description: >
  Shard a biomolecular model’s input features onto a CP device mesh with explicit placements, atom/token index mapping, padding, and masks. Use when adapting the production data pipeline for CP.
...
# shard-data-feats — produce DTensor features the CP model can consume
...
Hand off to `test` to build the data parity test (mirror
