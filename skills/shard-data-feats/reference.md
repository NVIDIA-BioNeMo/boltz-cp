# shard-data-feats — reference

All paths are relative to the repository selected by `BOLTZ_CP_REPO`. Tech guide section: **§1 Data Feature
Sharding**.
...
Padding gives every rank the identical buffer shapes required by
`torch.distributed` collectives and P2P, preventing hangs and invalid buffer accesses.
Two kinds:
