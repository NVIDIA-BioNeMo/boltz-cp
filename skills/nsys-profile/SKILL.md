...
name: nsys-profile
description: >
  Trace GPU compute and communication in a working CP workflow with NVIDIA Nsight Systems. Use to locate kernel, collective, and module timing bottlenecks; use mem-profile for allocation attribution.
...
# nsys-profile — Nsight Systems profiling for the CP workflow
...
- A CP workflow that runs end-to-end (forward for inference), verified by
  `test`; `docs/cp_infra.md` for topology + `world_size`.
...
- **Synthetic (fast, scale sweeps):** generate random features per the inventory
  ...
  **Glorot-uniform × small gain** (e.g. 0.1) so deep stacks stay numerically tame
  (also what `test` parity uses). Good for the CP-relevant subset (e.g.
